...
import { parseProtoDocument, flattenSymbols, getSymbolAtPosition } from './utils/protoParser';
...
export class ProtoCodeActionProvider implements vscode.CodeActionProvider {
  provideCodeActions(
    ...
    if (symbol?.kind === 'rpc') {
      ...
      if (!/option\s*\(\s*google\.api\.http\s*\)/.test(document.getText(symbol.range))) {
        ...
        const httpMethod = isGet && !isList ? 'get' : isCreate ? 'post' : isUpdate ? 'patch' : isDelete ? 'delete' : 'get';
        const insert = this.insertHttpOption(document, symbol.range, httpMethod, path, isCreate || isUpdate);
        if (insert) {
          const action = new vscode.CodeAction('Add (google.api.http)', vscode.CodeActionKind.QuickFix);
          action.edit = insert;
          action.diagnostics = context.diagnostics.filter((d) => d.range.intersection(symbol.range));
          actions.push(action);
        }
      }
    }
    ...
    if (symbol?.kind === 'message') {
      ...
      if (!/option\s*\(\s*google\.api\.resource\s*\)/.test(blockText)) {
        ...
        if (insert) {
          const action = new vscode.CodeAction('Add (google.api.resource)', vscode.CodeActionKind.QuickFix);
          action.edit = insert;
          actions.push(action);
        }
      }
    ...
    if (symbol?.kind === 'enum') {
      const blockText = document.getText(symbol.range);
      const firstValueMatch = blockText.match(/\{\s*([A-Za-z_][A-Za-z0-9_]*)\s*=\s*0/);
      ...
      if (firstValueMatch && firstValueMatch[1] !== unspecified) {
        ...
        if (insert) {
          const action = new vscode.CodeAction(`Add ${unspecified} = 0 as first enum value`, vscode.CodeActionKind.QuickFix);
          action.edit = insert;
          actions.push(action);
        }
      }
  ...
  private insertResourceOption(document: vscode.TextDocument, messageSymbol: { name: string; range: vscode.Range }): vscode.WorkspaceEdit | undefined {
    const edit = new vscode.WorkspaceEdit();
    ...
    const toInsert = indent + '  option (google.api.resource) = {\n' + indent + '    type: "example.com/' + typeName + '";\n' + indent + '    pattern: "' + pattern + '";\n' + indent + '  };\n';
    edit.insert(document.uri, new vscode.Position(insertLine, 0), toInsert);
    return edit;
  }

  private insertUnspecifiedEnum(document: vscode.TextDocument, enumSymbol: { name: string; range: vscode.Range }): vscode.WorkspaceEdit | undefined {
    const edit = new vscode.WorkspaceEdit();
