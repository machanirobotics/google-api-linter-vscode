...
import { findGapiConfigFile, findGapiConfigFileInFolder } from './utils/configReader';
import { DIAGNOSTIC_SOURCE } from './constants';
import { scanWorkspaceProto, LocationItem } from './protoScanner';

export type ProtoTreeNode =
  | { kind: 'status'; label: string; version?: string; detail?: string; icon: string }
  | { kind: 'init'; label: string; detail: string; icon: string; folderUri?: vscode.Uri }
  | { kind: 'file'; uri: vscode.Uri; errorCount: number; warningCount: number; folderName?: string }
  | { kind: 'diagnostic'; uri: vscode.Uri; range: vscode.Range; message: string; severity: vscode.DiagnosticSeverity; code?: string | number }
  | { kind: 'section'; id: 'rpcs' | 'resources' | 'mcp'; label: string; count: number; icon: string }
  | { kind: 'location'; item: LocationItem }
  | { kind: 'folder'; name: string; uri: vscode.Uri }
  | { kind: 'action'; command: string; label: string; icon: string };

export class ProtoTreeDataProvider implements vscode.TreeDataProvider<ProtoTreeNode> {
  private readonly _onDidChangeTreeData = new vscode.EventEmitter<ProtoTreeNode | undefined | void>();
  readonly onDidChangeTreeData = this._onDidChangeTreeData.event;
  private scanCache: { rpcs: LocationItem[]; resources: LocationItem[]; mcp: LocationItem[] } | null = null;
  ...
  private async getScan(): Promise<{ rpcs: LocationItem[]; resources: LocationItem[]; mcp: LocationItem[] }> {
    if (this.scanCache) return this.scanCache;
    const root = vscode.workspace.workspaceFolders?.[0]?.uri;
    this.scanCache = root ? await scanWorkspaceProto(root) : { rpcs: [], resources: [], mcp: [] };
  ...
  getTreeItem(element: ProtoTreeNode): vscode.TreeItem {
    ...
    if (element.kind === 'file') {
      ...
      const label = folderName != null ? vscode.workspace.asRelativePath(uri, false).replace(/^[^/]+?\//, '') : vscode.workspace.asRelativePath(uri);
      const hasDiag = errorCount > 0 || warningCount > 0;
      const item = new vscode.TreeItem(
        label,
        hasDiag ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None
      );
      item.resourceUri = uri;
      item.description = hasDiag ? `${errorCount} error(s), ${warningCount} warning(s)` : 'OK';
    ...
    if (element.kind === 'diagnostic') {
      ...
      item.iconPath = new vscode.ThemeIcon(
        element.severity === vscode.DiagnosticSeverity.Error ? 'error' : 'warning'
      );
      item.command = {
  ...
  async getChildren(element?: ProtoTreeNode): Promise<ProtoTreeNode[]> {
    ...
    if (element?.kind === 'section') {
      ...
      if (element.id === 'rpcs') return scan.rpcs.map((item) => ({ kind: 'location' as const, item }));
      if (element.id === 'resources') return scan.resources.map((item) => ({ kind: 'location' as const, item }));
      if (element.id === 'mcp') return scan.mcp.map((item) => ({ kind: 'location' as const, item }));
      return [];
    }
    ...
    if (element?.kind === 'folder') {
      ...
      for (const uri of protoUris) {
        ...
        const fromUs = diagnostics.filter((d) => d.source === DIAGNOSTIC_SOURCE);
        const errorCount = fromUs.filter((d) => d.severity === vscode.DiagnosticSeverity.Error).length;
        const warningCount = fromUs.filter((d) => d.severity === vscode.DiagnosticSeverity.Warning).length;
        fileNodes.push({ kind: 'file', uri, errorCount, warningCount, folderName: element.name });
      }
      fileNodes.sort((a, b) => {
    ...
    if (scan.rpcs.length > 0) {
      roots.push({ kind: 'section', id: 'rpcs', label: 'RPCs', count: scan.rpcs.length, icon: 'symbol-method' });
    }
    if (scan.resources.length > 0) {
      roots.push({ kind: 'section', id: 'resources', label: 'Resources', count: scan.resources.length, icon: 'symbol-class' });
    }
    if (scan.mcp.length > 0) {
      roots.push({ kind: 'section', id: 'mcp', label: 'MCP', count: scan.mcp.length, icon: 'symbol-interface' });
    }
    ...
    roots.push(
      { kind: 'action', command: 'googleApiLinter.lintWorkspace', label: 'Lint All Proto Files', icon: 'play' },
      { kind: 'action', command: 'googleApiLinter.lintCurrentFile', label: 'Lint Current File', icon: 'go-to-file' },
      { kind: 'action', command: 'googleApiLinter.createConfig', label: 'Create Config File', icon: 'settings-gear' },
      { kind: 'action', command: 'googleApiLinter.initWorkspace', label: 'Initialize Proto Workspace', icon: 'folder-opened' },
      { kind: 'action', command: 'googleApiLinter.restart', label: 'Restart', icon: 'debug-restart' }
    );
...
export function registerProtoView(
  ...
  context.subscriptions.push(
    vscode.window.registerTreeDataProvider('googleApiLinter.views.proto', treeDataProvider)
  );
  ...
  const watcher = vscode.workspace.createFileSystemWatcher('**/workspace.protobuf.yaml');
