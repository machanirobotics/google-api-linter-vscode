...
export class ProtoReferenceProvider implements vscode.ReferenceProvider {
  async provideReferences(
    ...
    const nameAtPosition = document.getText(document.getWordRangeAtPosition(position));
          ...
          for (const s of flat) {
            if (s.kind !== 'rpc' && simpleName(s.name) === targetSimple) {
              const alreadyHasDef = locations.some(
                (l) => l.uri.toString() === uri.toString() && l.range.isEqual(s.selectionRange)
              );
              if (!alreadyHasDef) locations.push(new vscode.Location(uri, s.selectionRange));
            }
          }
        }
