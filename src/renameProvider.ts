...
import { parseProtoDocument, flattenSymbols, collectTypeReferences, simpleName } from './utils/protoParser';
...
export class ProtoRenameProvider implements vscode.RenameProvider {
  async provideRenameEdits(
    ...
    if (!symbol || (symbol.kind !== 'message' && symbol.kind !== 'service' && symbol.kind !== 'enum' && symbol.kind !== 'rpc')) {
      return null;
    }
    ...
    for (const ref of refs) {
      if (simpleName(ref.typeName) === targetSimple && !ref.range.contains(position)) {
      ...
      try {
        ...
        for (const s of otherFlat) {
          if (s.kind !== 'rpc' && simpleName(s.name) === targetSimple) {
            const already = locations.some((l) => l.uri.toString() === uri.toString() && l.range.isEqual(s.selectionRange));
            if (!already) locations.push(new vscode.Location(uri, s.selectionRange));
          }
        }
