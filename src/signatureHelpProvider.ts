 ...
 */
export class ProtoSignatureHelpProvider
	implements vscode.SignatureHelpProvider
{
	provideSignatureHelp(
		...
		if (
			/option\s*\(\s*google\.api\.http\s*\)\s*=\s*\{/.test(prefix) ||
			/option\s*\(\s*google\.api\.http\s*\)\s*=\s*\{\s*\w*/.test(prefix)
		) {
	...
	private rpcRequestSignature(
		...
		sig.parameters = [
			new vscode.ParameterInformation(
				"stream? Request",
				"Request message type (e.g. GetFooRequest). Prefix with `stream` for client streaming.",
			),
			new vscode.ParameterInformation(
				"returns (stream? Response)",
				"Response message type. Prefix with `stream` for server streaming.",
			),
		];
	...
	private rpcResponseSignature(
		...
		const sig = new vscode.SignatureInformation(
			"returns (stream? Response)",
			new vscode.MarkdownString(
				"**Response type** for the RPC. Use `stream ResponseType` for server streaming.",
			),
		);
		sig.parameters = [
			new vscode.ParameterInformation(
				"stream? Response",
				"Response message type (e.g. Foo).",
			),
		];
		const help = new vscode.SignatureHelp();
		help.signatures = [sig];
	...
	private httpOptionSignature(
		...
		sig.parameters = [
			new vscode.ParameterInformation(
				'get: "/path"',
				"GET path template. Use `{name}` for path variables.",
			),
			...
			new vscode.ParameterInformation(
				'body: "field_name"',
				"Request field whose value is the HTTP body.",
			),
		];
		const help = new vscode.SignatureHelp();
		help.signatures = [sig];
