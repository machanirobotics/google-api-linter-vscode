...
export type ProtoSymbolKind = 'message' | 'service' | 'enum' | 'rpc' | 'field' | 'enumValue';
...
const RE_RPC = /^\s*rpc\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(\s*(stream\s+)?([A-Za-z_.][A-Za-z0-9_.]*)\s*\)\s*returns\s*\(\s*(stream\s+)?([A-Za-z_.][A-Za-z0-9_.]*)\s*\)/;
const RE_ONEOF = /^\s*oneof\s+([A-Za-z_][A-Za-z0-9_]*)\s*\{?/;

function findMatchingBrace(lines: string[], startLine: number, openCh: '{', closeCh: '}' = '}'): number {
  let depth = 0;
...
function rangeFromLines(startLine: number, startChar: number, endLine: number, endChar: number): vscode.Range {
  return new vscode.Range(startLine, startChar, endLine, endChar);
}

function lineRange(document: vscode.TextDocument, lineIndex: number, startChar: number, endChar: number): vscode.Range {
 ...
 */
export function parseProtoDocument(document: vscode.TextDocument): ProtoSymbol[] {
        ...
        for (let j = i + 1; j < endLine; j++) {
          ...
          if (rpcMatch && !RE_COMMENT.test(lines[j])) {
            ...
            children.push({
              ...
              selectionRange: lineRange(document, j, rpcStart, rpcStart + rpcName.length),
            });
          }
        }
      }
...
export function getSymbolAtPosition(symbols: ProtoSymbol[], position: vscode.Position): ProtoSymbol | undefined {
...
export function collectTypeReferences(document: vscode.TextDocument): { typeName: string; range: vscode.Range }[] {
  const refs: { typeName: string; range: vscode.Range }[] = [];
  const lines = document.getText().split('\n');
  const reFieldType = /\b(?:optional|repeated|required|stream)?\s*(\\.?[A-Za-z_][A-Za-z0-9_.]*)\s+(\w+)\s*=\s*\d+/;
  const reRpc = /\brpc\s+\w+\s*\(\s*(?:stream\s+)?([A-Za-z_.][A-Za-z0-9_.]*)\s*\)\s*returns\s*\(\s*(?:stream\s+)?([A-Za-z_.][A-Za-z0-9_.]*)\s*\)/;
  const reMap = /\bmap\s*<\s*(\\.?[A-Za-z_][A-Za-z0-9_.]*)\s*,\s*(\\.?[A-Za-z_][A-Za-z0-9_.]*)\s*>/;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    while ((m = re1.exec(line))) {
      ...
      if (/^(double|float|int32|int64|uint32|uint64|sint32|sint64|fixed32|fixed64|sfixed32|sfixed64|bool|string|bytes)$/.test(typeName)) continue;
      const typeStart = m.index + m[0].indexOf(m[1]);
      refs.push({ typeName, range: new vscode.Range(i, typeStart, i, typeStart + m[1].length) });
    }
    const rpcM = line.match(reRpc);
    if (rpcM) {
      const reqStart = line.indexOf(rpcM[1]);
      refs.push({ typeName: rpcM[1], range: new vscode.Range(i, reqStart, i, reqStart + rpcM[1].length) });
      const resStart = line.indexOf(rpcM[2]);
      refs.push({ typeName: rpcM[2], range: new vscode.Range(i, resStart, i, resStart + rpcM[2].length) });
    }
    const mapM = line.match(reMap);
    if (mapM) {
      const kStart = line.indexOf(mapM[1]);
      refs.push({ typeName: mapM[1].replace(/^\./, ''), range: new vscode.Range(i, kStart, i, kStart + mapM[1].length) });
      const vStart = line.indexOf(mapM[2], kStart + mapM[1].length);
      refs.push({ typeName: mapM[2].replace(/^\./, ''), range: new vscode.Range(i, vStart, i, vStart + mapM[2].length) });
    }
  }
  return refs;
}
